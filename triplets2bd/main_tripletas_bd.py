...
if __name__ == "__main__":
    ...
    p.add_argument("--triplets-json", type=str)
    p.add_argument("--triplets-file", type=str)
    args = p.parse_args()
    ...
    start = time.perf_counter()  # Inicio del contador
    res = run_triplets_to_bd(triplets, opts)
    elapsed = time.perf_counter() - start  # Tiempo total

    print(f"Backend={res.backend} | modo={res.mode} | reset={'sí' if res.reset else 'no'} | run_id={res.run_id} | ejecutadas={res.executed_statements} | tiempo={elapsed:.2f}s")
